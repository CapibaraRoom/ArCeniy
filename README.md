...
**АрСений** - модульная система на базе **Porteus+Arch** с простой установкой за счет использования aufs. Для тех, кому нужна стабильность, настоятельно рекомендуется рассмотреть официальные дистрибутивы [Porteus](https://forum.porteus.org/). В данном проекте целью стоит запуск стороннего образа системы ради расширения используемых концепций и инструментов. Рассчитан на тех, кто хочет простой работы с арчем, [кстати](https://cs12.pikabu.ru/post_img/2021/06/27/7/1624790613136826344.jpg).

Во время запуска АрСений выгружает в RAM файловую систему из iso дистрибутива, а так же накладывает на неё фрагменты, сформированные в модулях формата **xzm** (сжатый squashfs). Раздел *home/* монтируется без выгрузки. Такой подход позволяет поддерживать систему в чистом и безопасном состоянии.
...
└── filesystem/            -- директория файловой системы (или корень раздела)
    ├── boot/              -- ядро с поддержкой aufs и initrd для запуска
...
> * Автор не гарантирует стабильность работы системы до завершения работ (до выхода релиза >1.0.0). В случае возникновения проблем просьба написать о них и надеяться на решение. Приходите с личным авокадо &#x1F951; - я приду со своим &#x1F951;.
...
- ~~Изменение системы запуска sysvinit -> grub (такой подход позволить запускать даже нераспакованые системы прямо из iso).~~
